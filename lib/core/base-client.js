...
function buildLoginPacket(cmd, body, type = 2) {
    ...
    let sso = new writer_1.default()
        ...
        .writeBytes(ksid)
        .writeWithLength(this.device.qImei16 || constants_1.BUF0)
        .read())
...
function calcPoW(data) {
    ...
    if (typ === 2 && tgt.length === 32) {
        ...
        let hash = (0, crypto_1.createHash)("sha256").update(Buffer.from(tmp.toString(16).padStart(256, "0"), "hex")).digest();
        while (Buffer.compare(hash, tgt)) {
            tmp++;
            hash = (0, crypto_1.createHash)("sha256").update(Buffer.from(tmp.toString(16).padStart(256, "0"), "hex")).digest();
            cnt++;
        }
        ok = true;
...
function decodeLoginResponse(payload) {
    ...
    const t = readTlv(r);
    if (t[0x546])
        this.sig.t547 = calcPoW.call(this, t[0x546]);
    if (type === 204) {
