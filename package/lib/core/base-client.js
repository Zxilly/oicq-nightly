...
function calcPoW(data) {
    ...
    if (typ === 2 && tgt.length === 32) {
        ...
        let hash = (0, crypto_1.createHash)("sha256").update(Buffer.from(tmp.toString(16), "hex")).digest();
        while (Buffer.compare(hash, tgt)) {
            tmp++;
            hash = (0, crypto_1.createHash)("sha256").update(Buffer.from(tmp.toString(16), "hex")).digest();
            cnt++;
        }
        ok = true;
