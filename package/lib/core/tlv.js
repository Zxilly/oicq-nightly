...
const encryption_1 = require("../internal/encryption");
...
const map = {
    ...
    0x544: function (v, subCmd) {
        ...
        return new writer_1.default().writeBytes((0, encryption_1.sign)((new Date()).getTime(), salt.read()));
    },
    0x545: function () {
        const writer = new writer_1.default();
        if (this.device.qImei16)
            writer.writeBytes(Buffer.from(this.device.qImei16));
        else
            writer.writeBytes(Buffer.from(this.device.imei));
        return writer;
    },
    0x547: function () {
